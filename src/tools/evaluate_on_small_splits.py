...
def generate_reports(
    ...
    for i, metric in enumerate(metrics):
        ...
        bars = axes[i].bar(
            range(len(results[metric])),
            results[metric],
            color="mediumseagreen",
        )
        mu = np.mean(results[metric])
        sigma = np.std(results[metric])
        axes[i].text(5, 0.8, f"mean={mu:.2f}, \n std={sigma:.4f}")
        axes[i].set_ylim(0, 1.0)
        axes[i].set_title(metric)
        stylize_bars(bars, axes[i], "k")

    f.savefig(
        os.path.join(work_dir, "evaluate_on_small_splits.pdf"),
        bbox_inches="tight",
    )
...
def evaluate(dataset, out_dir):
    ...
    for model_cfg in MODEL_CFGS.keys():
        ...
        for m in range(10):  # range(20, 120, 20):
            _ = test(
                package="mmdet",
                ...
                other_args=(
                    ...
                    f"data.test.ann_file=data/processed/coco-subsets/val_{m}.json",
